        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {SERVICES.map((service, index) => (
            <Reveal key={index} delay={index * 100}>
              <GlowCard className="h-full flex flex-col items-start group hover:border-brand-action/50 transition-all duration-300 bg-white" glowColor="teal">
                <div className="w-14 h-14 rounded-xl bg-surface-secondary flex items-center justify-center text-brand-teal mb-6 group-hover:bg-brand-petrol group-hover:text-brand-lime transition-colors duration-300 shadow-sm relative z-20">
                  {service.icon}
                </div>
                <h4 className="text-xl font-bold text-text-primary mb-3 relative z-20">{service.title}</h4>
